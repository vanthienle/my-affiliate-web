...
# ==========================================
# 1. TRANG CHỦ (GIAO DIỆN MỚI - TÔNG XANH)
# ==========================================
...
def index():
    ...
    <body class="bg-[#F4F7FB] text-slate-800 font-sans min-h-screen flex flex-col justify-between relative overflow-x-hidden">
        
        <!-- Hình trang trí phông nền -->
        <div class="absolute top-0 left-0 w-full h-96 bg-gradient-to-b from-blue-100/60 to-transparent -z-10"></div>
        <div class="absolute -top-20 -right-20 w-96 h-96 bg-indigo-200/40 rounded-full blur-3xl -z-10"></div>
        ...
        <main class="max-w-4xl mx-auto px-4 py-16 text-center flex-1 flex flex-col items-center justify-center">
            ...
            <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-[#0068FF] text-xs font-semibold mb-6">
            ...
            <h1 class="text-4xl md:text-6xl font-extrabold text-slate-900 mb-5 tracking-tight leading-tight">
                ...
                <span class="text-[#0068FF] block mt-2">Tự động & Nhanh chóng</span>
            </h1>

            <p class="text-slate-500 text-base md:text-lg mb-10 max-w-xl mx-auto">
            ...
            <div class="w-full bg-white rounded-[2rem] p-5 md:p-8 shadow-2xl shadow-blue-900/5 border border-slate-100 text-left relative overflow-hidden">
                
                <!-- Bảng các sàn hỗ trợ -->
                <div class="flex items-center gap-6 mb-5 px-1">
                    <span class="flex items-center gap-2 text-sm font-semibold text-orange-600 bg-orange-50 px-3 py-1.5 rounded-lg">
                        <i class="fa-solid fa-bag-shopping"></i> Shopee
                    </span>
                    <span class="flex items-center gap-2 text-sm font-semibold text-slate-900 bg-slate-100 px-3 py-1.5 rounded-lg">
                        <i class="fa-brands fa-tiktok"></i> TikTok
                    </span>
                    <span class="flex items-center gap-2 text-sm font-semibold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg">
...
def login_page():
    ...
    <body class="bg-[#F4F7FB] text-slate-800 min-h-screen flex flex-col items-center justify-center p-4 relative">
        <div class="absolute top-0 w-full h-64 bg-[#0068FF] -z-10" style="clip-path: polygon(0 0, 100% 0, 100% 60%, 0% 100%);"></div>
        ...
        <div class="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-slate-100">
            ...
            <p class="text-slate-500 text-sm mb-6">Nhập Mã theo dõi được Bot cấp trong nhóm Zalo (Vd: <code class="text-[#0068FF] bg-blue-50 px-2 py-0.5 rounded font-bold">zalo_uid</code>).</p>

            <form id="loginForm" class="space-y-4">
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Mã Tra Cứu (ID)</label>
                    <input type="text" id="trackingCode" placeholder="Nhập mã của bạn..." required 
                           class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#0068FF] focus:ring-4 focus:ring-blue-500/10 transition text-sm font-medium">
                </div>
                
                <div id="errMsg" class="hidden text-red-600 text-xs bg-red-50 p-3 rounded-xl border border-red-100"></div>
...
def orders_page():
    ...
    <body class="bg-[#F4F7FB] text-slate-800 min-h-screen p-4 md:p-8">
        <div class="max-w-4xl mx-auto">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Báo Cáo Hoa Hồng</h1>
                    <p class="text-slate-500 text-sm mt-1">ID Đang truy cập: <span id="userCode" class="text-[#0068FF] font-mono font-bold bg-blue-50 px-2 py-0.5 rounded"></span></p>
                </div>
                <div class="flex gap-2 w-full sm:w-auto">
                    <a href="/" class="flex-1 sm:flex-none justify-center px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm rounded-xl font-medium transition flex items-center gap-2">
                        <i class="fa-solid fa-house"></i> Home
                    </a>
                    <a href="/login" class="flex-1 sm:flex-none justify-center px-4 py-2 bg-[#0068FF] hover:bg-blue-600 text-white text-sm rounded-xl font-medium transition flex items-center gap-2">
                        <i class="fa-solid fa-right-from-bracket"></i> Đăng xuất
                    </a>
                </div>
            </div>

            <div class="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-lg shadow-slate-200/40">
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm text-slate-600">
                        <thead class="bg-slate-50 text-xs uppercase text-slate-500 border-b border-slate-100 font-semibold">
                            <tr>
                        ...
                        <tbody id="ordersTable" class="divide-y divide-slate-100">
                            <tr><td colspan="5" class="p-8 text-center text-slate-400">Đang tải dữ liệu...</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                if (data.success && data.orders.length > 0) {
                    tbody.innerHTML = data.orders.map(o => `
                        <tr class="hover:bg-slate-50 transition">
                            <td class="p-5 font-mono text-xs text-slate-400">${o.order_id}</td>
                            <td class="p-5 font-medium text-slate-800 line-clamp-2">${o.product_name}</td>
                            <td class="p-5">${Number(o.price).toLocaleString()}đ</td>
                            <td class="p-5 text-emerald-600 font-bold">+${Number(o.commission).toLocaleString()}đ</td>
                            <td class="p-5">
                                <span class="px-3 py-1.5 rounded-lg text-xs font-semibold ${o.status === 'Thành công' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100'}">
                                    ${o.status}
                                </span>
                            </td>
                        </tr>
                    ...
                    tbody.innerHTML = `<tr><td colspan="5" class="p-10 text-center text-slate-500 bg-slate-50/50">Không tìm thấy đơn hàng nào trong 30 ngày qua cho mã <b>${code}</b></td></tr>`;
                }
            }
            if (code) loadOrders();
        </script>
